...
class MiningEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # [truck_x, truck_y, truck_sin, truck_cos]
        # truck_low = np.array([250, 300, math.sqrt(3)/2, -1/2])
        # truck_high = np.array([650, 480, 1, 1/2])
        # truck_range = truck_high - truck_low

        # [e_x, e_y, ep_sin, ep_cos, ea_sin, ea_cos]
        # excvtr_low = np.array([300, 100, -1/2, math.sqrt(3)/2, math.sqrt(2)/2, -math.sqrt(2)/2])
        # excvtr_high = np.array([600, 200, 1/2, 1, 1, math.sqrt(2)/2])
        # excvtr_range = excvtr_high - excvtr_low

        # [truck_x, truck_y, truck_sin, truck_cos, e_x, e_y, ep_sin, ep_cos, ea_sin, ea_cos]
        # e = excavator, ep = excavator platform, ea = excavator arm
        ...
        self.observation_space = [spaces.Box(np.zeros(10), np.ones(10), dtype=np.float32),
                                  spaces.Box(np.zeros(10), np.ones(10), dtype=np.float32)]
        ...
        self.action_space = [spaces.Box(np.zeros(2), np.ones(2), dtype=np.float32),
                             spaces.Box(np.zeros(2), np.ones(2), dtype=np.float32)]
    ...
    def reset(self):
        ...
        # Scale sampled state
        scaled_state = self._get_scaled_state()
        obsv = [scaled_state, scaled_state]
...
def main():
    ...
    for i in range(60000):
        ...
        while not done:
            ...
            # action = [np.array([0.5, 0]), np.array([0.5, 0.8])]
